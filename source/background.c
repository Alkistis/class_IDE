...
int background_derivs(
  ...
  if (pba->has_scf == _TRUE_){
    ...
    Zbg = -y[pba->index_bi_phi_prime_scf]/y[pba->index_bi_a];
    Fz=2.*pba->scf_veta*Zbg;
    Fzz=2.*pba->scf_veta;

    dy[pba->index_bi_phi_prime_scf] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*y[pba->index_bi_phi_prime_scf]
    -3.*y[pba->index_bi_a]*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*(Fz-Zbg)/(1.-Fzz)
    -y[pba->index_bi_a]*y[pba->index_bi_a]*dV_scf(pba,y[pba->index_bi_phi_scf])/(1.-Fzz);
...
//  return  pow(phi - scf_B,  scf_alpha) +  scf_A; IDE modification
  return  scf_A;
}
...
double dV_p_scf(
  ...
  //return   scf_alpha*pow(phi -  scf_B,  scf_alpha - 1); IDE modification
  return 0.0;
}
...
double ddV_p_scf(
  ...
  //return  scf_alpha*(scf_alpha - 1.)*pow(phi -  scf_B,  scf_alpha - 2); IDE modification
  return 0.0;
}
